...
// FIXED: Sum all metas (don't divide by turnos)
...
async function main() {
  ...
  for (const [key, data] of dadosAggregated) {
    ...
    // FIXED: Don't divide by number of turnos
  ...
  console.log('\n=== ESPERADO ===');
  console.log('Inscritos: 8099 / 14268');
  console.log('Mat Fin: 2688 / 4999');
  console.log('Fin Doc: 2316 / 4749');
  console.log('Mat Acad: 692 / 3609');
  
  // Wait - the totals for meta are being counted multiple times!
  // Each turma with the same campus+curso gets the SAME meta
  // So we need to count meta only once per campus+curso, not per turma
  
  // Recalculate totals correctly
  let uniqueMetas = new Map<string, any>();
  for (const [key, meta] of metasAggregated) {
    uniqueMetas.set(key, meta);
  }
  
  let correctTotalInscritosMeta = 0;
  let correctTotalMatFinMeta = 0;
  let correctTotalFinDocMeta = 0;
  let correctTotalMatAcadMeta = 0;
  
  for (const [key, meta] of uniqueMetas) {
    correctTotalInscritosMeta += meta.inscritosMeta;
    correctTotalMatFinMeta += meta.matFinMeta;
    correctTotalFinDocMeta += meta.finDocMeta;
    correctTotalMatAcadMeta += meta.matAcadMeta;
  }
  
  console.log('\n=== TOTAIS CORRIGIDOS (contagem única por campus+curso) ===');
  console.log('Inscritos:', totalInscritosAtual, '/', Math.round(correctTotalInscritosMeta * 100) / 100);
  console.log('Mat Fin:', totalMatFinAtual, '/', Math.round(correctTotalMatFinMeta * 100) / 100);
  console.log('Fin Doc:', totalFinDocAtual, '/', Math.round(correctTotalFinDocMeta * 100) / 100);
  console.log('Mat Acad:', totalMatAcadAtual, '/', Math.round(correctTotalMatAcadMeta * 100) / 100);
  
  // Save to file with CORRECT totals
  const dashboardData = {
    totais: {
      inscritosAtual: totalInscritosAtual,
      inscritosMeta: Math.round(correctTotalInscritosMeta * 100) / 100,
      matFinAtual: totalMatFinAtual,
      matFinMeta: Math.round(correctTotalMatFinMeta * 100) / 100,
      finDocAtual: totalFinDocAtual,
      finDocMeta: Math.round(correctTotalFinDocMeta * 100) / 100,
      matAcadAtual: totalMatAcadAtual,
      matAcadMeta: Math.round(correctTotalMatAcadMeta * 100) / 100,
      totalTurmas: portfolio.length,
      turmasComDados: dadosAggregated.size,
      inscritosPercent: correctTotalInscritosMeta > 0 ? Math.round((totalInscritosAtual / correctTotalInscritosMeta) * 100) : 0,
      matFinPercent: correctTotalMatFinMeta > 0 ? Math.round((totalMatFinAtual / correctTotalMatFinMeta) * 100) : 0,
      finDocPercent: correctTotalFinDocMeta > 0 ? Math.round((totalFinDocAtual / correctTotalFinDocMeta) * 100) : 0,
      matAcadPercent: correctTotalMatAcadMeta > 0 ? Math.round((totalMatAcadAtual / correctTotalMatAcadMeta) * 100) : 0,
    },
    turmas: turmasData,
  };
